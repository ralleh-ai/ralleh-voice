...
def create_app():
    ...
    async def voice_ws(ws: WebSocket):
        ...
        async def emit_turn_output(turn: TurnState, result: TurnOutput) -> None:
            ...
            tts_encoding = getattr(pipeline.tts, "event_encoding", "base64-text-placeholder")
            tts_sample_rate = getattr(pipeline.tts, "event_sample_rate", None)
            for idx, out in enumerate(result.audio_chunks):
                if tts_encoding != "base64-text-placeholder":
                    ...
                    await send_event(EVENT_AUDIO_OUT_CHUNK, payload)
                    continue
